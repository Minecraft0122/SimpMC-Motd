...
消息查询和后台采样默认复用最近 `30` 秒的状态，避免短时间内重复 ping 同一服务器；可通过 `sample_reuse_seconds` 调整，设为 `0` 可关闭。插件默认不发送 Minecraft 协议中的额外 latency ping 包；确需记录延迟时，可在页面中开启“发送延迟 Ping”。
...
状态图会尽量保留 Minecraft MOTD 中的 JSON 颜色和 `§` 颜色码。时间轴和最后采样时间固定使用 `UTC+8 / Asia/Shanghai`，不跟随服务器系统时区。X 轴按真实采样时间窗显示；Y 轴顶部按历史在线人数最大值计算，除顶部最大值外，可见刻度使用 `5` 的倍数。历史记录不足两条时图表区会变灰；当前服务器无法连接时，图表区和离线状态会使用红色提示。
